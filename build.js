...
function splitSpecial(data) {
  ...
  if (offset < end) cells.push(cell(data, offset, end));
  if (data[end - 1] === CVS_SEPARATOR_CODE) cells.push('');
...
function cell(data, start, end) {
  ...
  for (; i < end; i++) {
    ...
    if (data[i] === CVS_ESCAPE_CODE && i + 1 < end && data[i + 1] === CVS_QUOTE_CODE) i++;
    if (y !== i) data[y] = data[i];
    y++;
  }
